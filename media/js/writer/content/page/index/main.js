...
(function($) {
	...
	var _ajaxUrls = {
		'getNews': '/Content/Index/news.html', //获取新闻列表
		'markNews': '/Content/Index/addReadnewsMark.html', //标记查看过新闻
		'addGuidedLog': '/Content/Authors/addGuideLog.html' //添加完成新手引导的记录
	};
		...
		'<%each list as item index%>',
			'<li>',
				'<a data-node="link" data-newsid="<%=item.IDX%>" class="link<%if item.mark !== 0%> visited<%/if%>" <%if item.Url == ""%>href="javascript:;"<%else%>href="<%=item.Url%>" target="_blank"<%/if%>>',
					...
					'<%if item.tag === "1"%>',
						'<img src="<%=imgUrl%>/content/new.png" width="16" height="16">',
					'<%else if item.tag === "2"%>',
						'<img src="<%=imgUrl%>/content/hot.png" width="16" height="16">',
					'<%else if item.tag === "3"%>',
						'<img src="<%=imgUrl%>/content/new.png" width="16" height="16"><img src="<%=imgUrl%>/content/hot.png" width="16" height="16">',
					'<%/if%>',
				'</a>',
