...
(function($) {
    ...
    //ajax地址列表
    var _ajaxUrls = {
        'checkBookTitle' : '/Home/Booknovels/ajaxCheckBooktitleExist.html' //检测书名是否存在
    };
    ...
    var _activityTpl = [
        '<% each list as item i%>',
            '<span class="radio">',
                '<input name="Articleid" class="radioclass" type="radio" value="<%=item["IDX"]%>" />',
            '</span>',
            '<label><%=item["Authorcontent"]%></label>',
            '<a class="readme blue" href="<%=item["Notice"]%>" target="_blank">《参赛须知》</a>',
        '<% /each %>'
    ].join('');
    ...
    function _bindEventByTagPopup(){
        ...
        var $tagList = $('#novelTagPopup_tagList'), //作品标签浮层中的标签列表
        ...
        $tagList.on('click', 'a', function(event) {
            ...
            if(tagBoxCount > 1){
                ...
                }else if($(this).siblings('a.act').length < _maxSelectedTagCount){ //最多只能选3个
                    $(this).addClass('act');
                }
            }
        });
    ...
    function _updateNovelSubCategorySelector() {
        ...
        for (var prop in novelCategoryList) {
            var mainCategory = novelCategoryList[prop];
            
            //对应的主分类
            if (mainCategory.CategoryId == novelMainCategoryId) {
        ...
        if (selectedMainCategoryInfo && selectedMainCategoryInfo.SubNode) {
            for (var key in selectedMainCategoryInfo.SubNode) {
                var subCategory = selectedMainCategoryInfo.SubNode[key],
