/**
 * 积分商城（积分兑换）
 * @namespace CS.page.integral.exchange
 * @update 2015-11-19
 */
(function($) {
    //外部依赖
    ...
    var _bookListTpl = [
        '<%each list as item index%>',
            '<option value="<%=item.CBID%>"><%=item.title%>（<%=item.novelStatusName%>）</option>',
        '<%/each%>'
    ].join('');
    ...
    var _hardworkExchangeListTpl = [
        '<%each list as item index%>',
            '<dd>',
                '<b><%=item.date.toString().substr(0, 4)%> <%=item.date.toString().substr(4)%>月</b>',
    ...
    var _ajaxUrls = {
        //全勤请假条
        'fullTime' : {
            'getBook' : "/Content/IntegralExchange/fullTimeNovelList.html", //获取小说下拉列表
            'getExchangeInfo' : "/Content/IntegralExchange/fullTimeParam.html", //获取兑换的相关信息
            'submitExchange' : "/Content/IntegralExchange/fullTimeExchange.html" //兑换
        },
        //创作保障请假条
        'safeguard' : {
            'getBook' : "/Content/IntegralExchange/safeguardNovelList.html",
            'getExchangeInfo' : "/Content/IntegralExchange/safeguardParam.html",
            'submitExchange' : "/Content/IntegralExchange/safeguardExchange.html"
        },
        //勤奋写作豁免权
        'hardwork' : {
            'getBook' : "/Content/IntegralExchange/diligenceNovelList.html",
            'getExchangeInfo' : "/Content/IntegralExchange/diligenceParam.html",
            'submitExchange' : "/Content/IntegralExchange/diligenceExchange.html"
        }
    };
    ...
    function _getBookList(exchangeType){
        ...
        _util.request({
            url: _ajaxUrls[exchangeType].getBook,
            type: 'get',
            dataType: 'json',
            data: {},
    ...
    function _getExchangeInfo($btn, exchangeType, bid){
        ...
        _util.request({
            url: _ajaxUrls[exchangeType].getExchangeInfo,
            type: 'get',
            dataType: 'json',
            data: requestData,
            success: function(json) {
    ...
    function _submitExchange($btn, exchangeType, bid, monthList){
        ...
        _util.updateBtnText($btn, 'loading');
        _util.request({
            url: _ajaxUrls[exchangeType].submitExchange,
            type: 'post',
            dataType: 'json',
            data: requestData,
            success: function(json) {
    ...
    function _renderExchangeInfoPopup(exchangeType, bid, data){
        ...
        if(exchangeType === 'hardwork'){
            ...
            var monthlist=[];
            var scoreSum=0;
            $.each(data.cutWordAllTodate,function(key,value){
               monthlist.push(value['date']);
               scoreSum += value['need_score'];    
        })

            var $list = $popup.find('[data-node="exchangeList"]'), //兑换列表
                tpl = _uiBinder.bindData(_hardworkExchangeListTpl, {'list': data.cutWordAllTodate}); //兑换列表的模板
            
            $list.html(tpl);

                //消耗积分
                $popup.find('[data-node="exchangeIntegral"]').text(scoreSum);
