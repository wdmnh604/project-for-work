...
var _CONF={
    ...
    DEV_HOST_NAME : 'localhost', //开发环境hostname
    DEV_HOST : 'http://localhost:63342/QQReader_MqqIphone_html/WebContent/1/', //开发环境页面地址

	//TEST_HOST_NAME : 'solomotest4.3g.qq.com',
    TEST_HOST_NAME : 'ptsolomo.reader.qq.com',
	TEST_HOST_NAME2 : '117.135.128.74',
	TEST_HOST : 'http://solomotest4.3g.qq.com/book_res/reader/ios/common/', //测试环境页面地址
	TEST_HOST2 : 'http://117.135.128.74/book_res/reader/ios/common/',
	//TEST_SERVER : 'http://bookcs.ios.reader.qq.com',	 //测试环境后台服务地址
	TEST_SERVER : 'https://ptbookios.reader.qq.com',	
	
	FORMAL_HOST_NAME : 'iyuedu.qq.com', //正式环境hostname
	FORMAL_HOST : 'http://iyuedu.qq.com/ios/common/', //正式环境页面地址
	FORMAL_SERVER : 'https://newios.reader.qq.com', //正式环境后台服务地址
	
	VERSION : 'common',//当前协议版本号

	DEFAULT_PREFIX : 'http://wfqqreader.3g.qq.com/cover/'
};
...
function pageDomain() {
    //开发环境
    if (window.location.hostname == _CONF.TEST_HOST_NAME || window.location.hostname == _CONF.LOCAL_HOST_NAME || param("dotest") == 1) {
        return _CONF.TEST_HOST_NAME + '/' + _CONF.VERSION;
    }
    try {
        var pageCname = location.href.substr(7, location.href.indexOf("/", 7) - 7);
        if (pageCname.length > 0 && pageCname.indexOf(".") >= 0) {
            return "http://" + pageCname + '/android/' + _CONF.VERSION;
        } else {
