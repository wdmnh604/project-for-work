...
(function(){
	...
	var _ajaxUrls = {
		'modifyPwd' : '/Home/Authors/authorPassUpdate.html', //更改vip管理密码
		'getMsgNotice': '/Home/Authors/getNewMessageNote.html', //获取消息提示信息
		'addLog': '/Home/Authors/addLog.html' //添加点击隐藏官方推荐的记录
	};
	...
	var _newMsgListTpl = [
		...
		'<%each list as item index%>',
			'<%if item.ReadStatus === 0%>',
				'<dd data-questionid="<%=item.MsgId%>">',
					'<span class="icon"></span>您发起的咨询有新的回复<a class="blue" href="<%=item.url%>">详情&gt;</a>',
				'</dd>',
			'<%/if%>',
		'<%/each%>',
		'<%if newMsgCount > 3%>',
			'<dd data-node="more" class="more"><a href="/Home/Authors/authorconsult.html">查看更多 &gt;</a></dd>',
		'<%/if%>'
	].join('');
