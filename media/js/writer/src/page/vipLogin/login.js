...
(function($) {
	...
	var _ajaxUrls = {
		'submitLogin': '/Home/VIP/authorLoginSubmit.html', //提交登录
		'findPWDSubmit': '/Home/VIP/findPWDSubmit.html', //提交电子邮箱
		'submitNewPwd': '/Home/VIP/updatePWD.html' //提交新密码
	...
	function _bindEvent(){
		...
		$('[data-node="closeBtn"]').on('click', function() {
         ...
         var $email     =     $('#email');
		//填写基本信息浮层：提交按钮
		$('#inputBaseInfoPopup_submitBtn').on('click', function() {
			_submitEmail($realname.val(),$cardtype.val(),$cardid.val(),$email.val());
			return false;
		});
	 ...
	 * 打开填写填写电子邮箱的浮层
	 */
	function _openInputBaseInfoPopup(){
		//清空输入框
     ...
     * @param  {string} email 电子邮箱地址
     */
    function _submitEmail(realname,cardtype,cardid,email){

               if(!realname || $.trim(realname).length === 0){
			_dialog.alert('请填写真实姓名');
			return;
		}
                if(!cardtype || $.trim(cardtype).length === 0){
			_dialog.alert('请选择证件类型');
			return;
		}
                if(!cardid || $.trim(cardid).length === 0){
			_dialog.alert('请填写证件号码');
			return;
		}
        ...
        _util.request({
            ...
            success: function(json) {
				...
				if(json.status ){
					_closePopup();
					//打开验证邮件已经发送的提示浮层
					_openValidateEmailPopup(json.datashow);
				}else{
