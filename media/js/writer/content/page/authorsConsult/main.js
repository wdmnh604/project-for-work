...
(function($) {
    ...
    var _receiveListTpl = [
		'<%each list as item index%>',
			'<li data-questionid="<%=item.id%>">',
				'<a data-node="lookBtn" data-questionid="<%=item.id%>" class="link<%if item.isRead !== 0%> visited<%/if%>" href="javascript:;">',
					'<span <%if item.isRead === 0%>class="on"<%/if%> ></span>',
					'<cite class="time"><%=item.replyTime%></cite>',
					'<em class="minLogo <%=item.siteLogo%>"></em>',
					'<b><%=item.siteName%><i class="line">|</i><%=item.reMessage%></b>',
				'</a>',
				'<div class="detailMsgBox">',
					'<a data-node="deleteBtn" data-type="receive" data-questionid="<%=item.id%>" class="icon del" href="javascript:;"></a>',
					'<p><%=item.reMessage%></p>',
				'</div>',
			'</li>',
		'<%/each%>'
    ...
    var _sendListTpl = [
		'<%each list as item index%>',
			'<li data-questionid="<%=item.id%>">',
				'<a data-node="lookBtn" class="link" data-questionid="<%=item.id%>" href="javascript:;">',
					'<span></span>',
					'<cite class="time"><%=item.sendTime%></cite>',
					'<em class="minLogo <%=item.siteLogo%>"></em>',
					'<b><%=item.siteName%><i class="line">|</i><%=item.message%></b>',
				'</a>',
				'<div class="detailMsgBox">',
					'<a data-node="deleteBtn" data-type="send" data-questionid="<%=item.id%>" class="icon del" href="javascript:;"></a>',
					'<p><%=item.message%></p>',
				'</div>',
			'</li>',
		'<%/each%>'
	...
	var _ajaxUrls = {
		'getReceiveList': '/Content/Authors/receiveList.html', //获取收件箱的咨询列表
		'getSendList': '/Content/Authors/sendList.html', //获取发件箱的咨询列表
		'sendConsult' : '/Content/Authors/sendMessage.html', //发送新咨询
		'markConsult': '/Content/Authors/markMessage.html', //标记查看过收件箱咨询
		'deleteConsult': '/Content/Authors/delMessage.html', //删除咨询
		'clearSendBox' : '/Content/Authors/clearSendBox.html', //清空发件箱
		'clearReceiveBox': '/Content/Authors/clearReceiveBox.html' //清空收件箱
	};
