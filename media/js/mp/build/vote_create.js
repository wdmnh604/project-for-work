define('vote_create',['Node','template','common_gulp-seajs-cmobo_7','Dialog','DateTime','Form'],function(require, exports, module) {
	...
	module.exports = new Node({
	    ...
	    events: {
	    	'#addQues:click': 'addQues',
	    	'.addOption:click': 'addOption',
	    	'.jsDelQues:click': 'delQues'
	    },
	    ...
	    addQues: function(event) {
	    	...
	    	// 当前索引
	    	var quesId = +addBtn.attr('data-index'), max = addBtn.attr('max');
	    	...
	    	addBtn.before(template('quesTemp', {
	    		quesId: quesId
	    	}));

	    	// 改变投票问题序号
	    	addBtn.attr('data-index', quesId + 1);

	    	if (max && quesId + 1 > max) {
	    		addBtn.addClass(DISABLED).addClass('gray');
	    	}

	    	// 最大个数限制
	    	addBtn.prev('.jsQuesBox').find('input[maxlength]').each(function() {
	    		$.validate.count(this);
	    	});
	    ...
	    addOption: function(event) {
	    	...
	    	// 当前索引
	    	var optionId = +addBtn.attr('data-index');
	    	// 最大个数
	    	var max = addBtn.attr('max');
	    	...
	    	// 当前rel指向的容器
	    	var containerId = addBtn.attr('data-rel');

	    	// 计算出quesId，替换非数值
	    	var quesId = containerId.replace(/\D/g, '');

	    	// 模板走起
	    	var container = $('#' + containerId).append(template('optionTemp', {
	    		quesId: quesId,
	    		optionId: optionId
	    	}));

	    	// 改变投票问题序号
	    	addBtn.attr('data-index', optionId + 1);

	    	if (max && optionId + 1 > max) {
	    		addBtn.addClass(DISABLED).addClass('gray');
	    	}

	    	// 最大个数限制
	    	container.find('.jsNewOption').removeClass('jsNewOption').find('input[maxlength]').each(function() {
	    		$.validate.count(this);
	    	});
	    },
	     ...
	     */
	    delQues: function(event) {
	    	// 元素
	    	var btnDel = event.el, btnAdd = $('#addQues');

	    	// 是否已经有填写内容
	    	var container = btnDel.parents('.jsQuesBox');

	    	if (container.length && btnAdd.length) {
	    		var isFill = false;

	    		// 当前的序号
	    		var index = btnAdd.attr('data-index');

	    		container.find('input').each(function() {
	    			var input = this, type = input.type;
	    			if (type != 'radio' && type != 'checkbox' && $.trim(input.value)) {
	    				isFill = true;
	    				// 退出循环
	    				return false;
	    			}
	    		...
	    		if (isFill == true) {
	    			new Dialog().confirm('此问题有一些选项你已经填写，删除后将无法恢复，确定删除？', {
	    				buttons: [{
	    					value: '删除',
	    					events: {
	    						click: function() {
	    							container.remove();
	    							btnAdd.attr('data-index', index*1 - 1);
	    		...
	    		success: function(json) {
	    			$.jsonHandle(json, function() {
	    				if (json.url) {
	    					...
	    					}).confirm((json.info || '投票新建成功！') + '您可以查看新建的投票或者继续创建。', {
	    						buttons: [{
	    							type: 'primary',
	    							value: '查看投票',
	    							events: {
	    					...
	    					$.lightTip.success(json.info || '投票新建成功！');
	    					setTimeout(function() {
	    						location.href = location.href.split('#')[0];
	    					}, 2000);
	    				}
...
define('vote',['Node','template','common_gulp-seajs-cmobo_7','Table','Dialog'],function(require, exports, module) {
	var Node = require('Node'),
	template = require('template');
	...
	module.exports = new Node({
	    ...
	    loadVoteData: function(id) {
	    	...
	    	var elPanel = $('#' + idPanel), 
	    		elForm = $('#' + idPanel + 'Form'),
	    		elTable = elPanel.find('table');

	    	if (elPanel.length && elForm.length) {
	    		var voteTable = new Table(elTable, {
	    			pageOptions: {
	    				every: elTable.attr('data-every') || self.every || 15
	    			},
	    			ajaxOptions: {
	    				url: elForm.attr('action'),
	    				type: elForm.attr('method') || 'get',
	    			...
	    			parse: function(json) {
	    				if (json.status == true) {
	    					return template(idPanel + 'Temp', json.data);
	    				}
	    			}
	    		});
	    	}
...
define('user_ana',['Node','template','common_gulp-seajs-cmobo_7','Tab'],function(require, exports, module) {
	var Node = require('Node'),
		template = require('template');
...
define('user',['Node','template','common_gulp-seajs-cmobo_7','Validate','Dialog','Table'],function(require, exports, module) {
	var Node = require('Node');
	template = require('template');
	var common = require('common_gulp-seajs-cmobo_7');
	...
	module.exports = new Node({
	    // 一些全局委托的事件处理，默认在'body'上
	    events: {
	    	'.userEditForm:submit': 'userMarkEdit',
	    	'.jsMarkEdit:click': 'showMarkForm',
	    	'.jsMeditInput:focus': function() {
	    		$(this).data('initValue', $(this).val());
	    	},
	    	'.jsMeditInput:blur': function() {
	    		var input = $(this), value = input.val();
	    		if (value == input.data('initValue')) {
	    			// 值没有改变，直接隐藏
	    			module.exports.hideMarkForm({
	    				el: input
	    			});
	    		} else {
	    ...
	    loadUserData: function(id) {
	    	...
	    	if (elPanel.length && elForm.length) {
	    		var userTable = new Table(elTable, {
	    			pageOptions: {
	    				every: elTable.attr('data-every') || self.every || 15
	    			},
	    			ajaxOptions: {
				...
				var elSearchForm = $('#mixSeaechForm');
				new Validate(elSearchForm, function() {
					userTable.ajax({
						data: $.queryToObject(elSearchForm.serialize())
					});
				});
		...
		userMarkEdit: function(event) {
			...
			// 输入框
			var input = form.find('.jsMeditInput');
			// id
			var id = input.attr('data-id');
			// 字符个数限制
			var maxlength = input.attr('maxlength') || 200;
			...
			if (input.length > maxlength) {
				$.validate.errorTip(input, '备注不能超过'+ maxlength +'个字符');
				return self;
			}
			...
			$.ajax({
				...
				success: function(json) {
					$.jsonHandle(json, function() {
						// 根据模板求得关键字
						var key = self.dataKey, keyId = self.idKey || 'id';

						// 更新模板数据
						if (self.data && self.data[key]) {
							$.each(self.data[key], function(i, obj) {
								if (obj[keyId] == id) {
							...
							// 模板重新渲染
							if (self.table) {
								self.table.el.table.find('tbody').html(template('allUserTemp', self.data));
							}
						} else {
							// 刷新，基本上不会出现，这里是防止意外发生的紧急功能处理
							location.href = location.href.split('#')[0];
						}
					});
				},
				complete: function() {
					...
					input.next('.icon').addClass('vh');
				}
			});

			input.attr(DISABLED, DISABLED);
			input.next('.icon').removeClass('vh');
		 ...
		 * @return {[type]} [description]
		 */
		showMarkForm: function(event) {
			var self = this, el = event.el;

			var parent = el.parents('.jsEditParent').hide();
			var form = el.parents('td').find('form').show(), input = form.find('.jsMeditInput');
		 ...
		 * @return {[type]} [description]
		 */
		hideMarkForm: function(event) {
			var self = this, input = event.el;

			var form = input.parents('form').hide();
			input.parents('td').find('.jsEditParent').show();
		...
		getDataKey: function() {
	    	...
	    	this.dataKey = $.trim(temp.split('}}')[0].replace(/{{\s*[a-z]+/, ''));
	    	var arrIdSp = temp.split(/data\-id=['"]{{\s*\$value\./i);
	    	if (arrIdSp.length > 1) {
	    		this.idKey = $.trim(arrIdSp[1].split('}}')[0]);
	    	}
		},
 ...
 * Created: 15-07-01
 */
define('DropPanel',['Drop'],function(require, exports, module) {
	...
	* ｝
	...
	var prefixGlobal = 'ui_', joiner = '_', prefixDropPanel = 'ui_dropanel_', SELECTED = 'selected', DISABLED = 'disabled'; 
 ...
 * Created: 15-06-30
 */
define('DropList',['Drop'],function(require, exports, module) {
    //require('plugin/jquery');
	...
	* }]
	...
	var prefixDropList = 'ui_droplist_', SELECTED = 'selected', DISABLED = 'disabled'; 
	...
	var DropList = function(trigger, data, options) {
		...
		target.delegate('a', 'click', function() {
			...
			if ($(this).hasClass(SELECTED) == false) {
				...
				params.onSelect.call(this, data[index], drop);
			}
 ...
 * Created: 15-06-30
 */
define('Drop_gulp-seajs-cmobo_74',['Follow'],function(require, exports, module) {
    //require('plugin/jquery');
...
define('ui',['Checkbox','DateTime','Dialog','Drop_gulp-seajs-cmobo_74','DropList','DropPanel','ErrorTip','LightTip','Loading','Pagination','Placeholder','Radio','Select','Tab','Tips','Validate','Table','Form'],function(require, exports, module) {
    var Checkbox = require('Checkbox'),
        DateTime = require('DateTime'),
        Dialog = require('Dialog'),
        Drop = require('Drop_gulp-seajs-cmobo_74'),
        DropList = require('DropList'),
        DropPanel = require('DropPanel'),
        ErrorTip = require('ErrorTip'),
        LightTip = require('LightTip'),
...
define('send',['Node','template','common_gulp-seajs-cmobo_7','Dialog','Table'],function(require, exports, module) {
	var Node = require('Node'),
		template = require('template');
	...
	module.exports = new Node({
	     ...
	     * @return {[type]} [description]
	     */
	    loadDataList: function(event) {
	    	var self = this;
	    	// 选项卡元素
	    	var elTab = event.el;

	    	// 如果已经加载过数据，就不重复加载了
	    	if (elTab.data('loaded') == true) {
	    		return;
	    	}

	    	// 加载数据
	    	var idPanel = elTab.attr('data-rel');
	    	// 一些元素
	    	var elPanel = $('#' + idPanel), 
	    		elForm = $('#' + idPanel + 'Form'),
	    		elTable = elPanel.find('table');
	    	...
	    	if (elPanel.length && elForm.length) {
	    		var sendTable = new Table(elTable, {
	    			pageOptions: {
	    				every: elTable.attr('data-every') || self.every || 15
	    			},
	    			ajaxOptions: {
	    			...
	    			events: {
	    				'.jsDel:click': function() {
	    					var btnDel = $(this);
	    					new Dialog().confirm('<h6>删除文章</h6>\
    						<p>一旦删除就无法恢复，是否删除这篇文章？</p>', {
    							buttons: [{
    								value: '删除',
    								events: {
    									click: function() {
    										var btn = $(this);
    										self.deleteDataList(btnDel, btn, function() {
    											// 重新请求数据
    											sendTable.ajax();
    										}, function() {
    											btn.siblings('a').trigger('click');
    										});
    									}
    								}
    							}, {}]
	     ...
	     * @return {[type]}     [description]
	     */
	    deleteDataList: function(btn, btnDialog, success, complete) {
	    	var self = this;
	    	...
	    	// 请求地址
	    	var url = btn && btn.attr('data-url');

	    	if (url && btn.hasClass(DISABLED) == false) {
	    		btnDialog.loading();

	    		$.ajax({
	    			url: url,
	    			type: 'post',
	    			data: $.queryToObject(btn.attr('data-params') || ''),
	    			success: function(json) {
	    				$.jsonHandle(json, success);
	    			},
	    			complete: function() {
	    				complete();
	    				btnDialog.unloading();
...
define('Pagination',function(require, exports, module) {		
	...
	var Pagination = function(container, options) {
		...
		container.delegate('a', 'click', function(event) {
			...
			page.show();
			// 点击回调			
			params.onClick.call(this, page, current);
	...
	Pagination.prototype.create = function(num, mode) {
		...
		// ui类名
		var CL = 'ui_page', prefixPage = CL + '_';
		...
		// 1. 首先是通用的前后翻页
		if (current > 1) {
			html = html + '<a href="'+ fnHref(current - 1) +'" class="'+ clPrev +'" data-page="'+ (current - 1) +'">'+ '前一页' +'</a>';
		} else {
			// 当前是第一页，自然不能往前翻页
			html = html + '<span class="'+ clPrev +'">'+ '前一页' +'</span>';
		}
		...
		// 3. 往后翻页
		if (current < total) {
			html = html + '<a href="'+ fnHref(current + 1) +'" class="'+ clNext +'" data-page="'+ (current + 1) +'">'+ '后一页' +'</a>';
		} else {
			html = html + '<span class="'+ clNext +'">'+ '后一页' +'</span>';
		}
 ...
 * Created: 15-06-28
 */
define('Table',['Pagination','Tips','Loading'],function(require, exports, module) {
    //require('plugin/jquery');

    // 复选框
 //    if (!window.addEventListener) {
	// 	require.async('../ui/Checkbox');
	// }
    //var DropList = require('../ui/DropList');
	...
	var CL = {
		...
		// 下面是分页相关的：
		//   总数据量
		total: 'table_num_length',
		//   每页系那是数目
		every: 'table_num_every',
		//   分页左侧容器
  ...
  //       });
	...
	Table.prototype.ajax = function(options) {
		...
		} else if (typeof this.ajaxOptions.data == 'string') {
			$.each(this.ajaxOptions.data.split('&'), function(index, parts) {
		...
		num.current = ajaxParams.data.page;
		...
		ajaxParams.success = function(data) {
			...
			if (!total && total !== 0) {
				total = data.data && data.data.count;
			}
			if (total || total == 0) {
				el.total.html(total);
		            ...
		            onClick: function(page, current) {
		            	...
		            	if ($.isFunction(self.callback.page)) {
			            	...
			            	if (/prev/.test(this.className)) {
			            		page.el.container.find('.ui_page_prev').addClass(LOADING);
			            	} else if (/next/.test(this.className)) {
			            		page.el.container.find('.ui_page_next').addClass(LOADING);
			            	} else {
			            		page.el.container.find('.ui_page_current').addClass(LOADING);
			            	}
...
define('message',['Node','template','common_gulp-seajs-cmobo_7','Dialog','Table','Form'],function(require, exports, module) {
	var Node = require('Node'),
		template = require('template');
	...
	module.exports = new Node({
	    ...
	    loadDataList: function(event) {
	    	...
	    	if (elPanel.length && elForm.length) {
	    		var msgTable = new Table(elTable, {
	    			pageOptions: {
	    				every: elTable.attr('data-every') || self.every || 15
	    			},
	    			ajaxOptions: {
	    			...
	    			events: {
	    				'.jsReply:click': function() {
	    					self.replyTrToggle($(this));

	    					// 列表的尺寸同步
	    					msgTable.el.size.unloading(0);
	    				},
	    				'.jsDelMsg:click': function() {
	    					var btnDel = $(this);

	    					// 是否有其他未发布的留言
	    					var isUnpublishMsg = false;
	    					elPanel.find('.jsArea').each(function() {
	    						if ($.trim(this.value) != '') {
	    							isUnpublishMsg = true;

	    							// 此回复面板展开
	    							$(this).parents('.jsReplyTr').show();
	    							// 列表的尺寸同步
	    							msgTable.el.size.unloading(0);
	    						}
	    					});
	    					...
	    					var delConfirm = function() {
	    						var btn = $(this);
								self.deleteReply(btnDel, btn, function() {
									btn.siblings('a').trigger('click');
	    					...
	    					if (isUnpublishMsg == true) {
	    						new Dialog().confirm('<p>你尚有未提交的回复信息，本次删除会刷新列表，你的回复会丢失，是否继续删除操作？</p>', {
	    							buttons: [{
	    								value: '继续',
	    								events: {
	    									click: delConfirm
	    						...
	    						new Dialog().confirm('<h6>删除留言</h6>\
	    						<p>一旦删除就无法恢复，是否删除这个留言？</p>', {
	    							buttons: [{
	    								value: '删除',
	    								events: {
	    									click: delConfirm
	    				...
	    				'.jsDelReply:click': function() {
	    					var btnDel = $(this);
	    					new Dialog().confirm('<h6>删除回复</h6>\
    						<p>一旦删除就无法恢复，是否删除这个回复？</p>', {
    							buttons: [{
    								value: '删除',
    								events: {
    									click: function() {
	    				...
	    				'.jsFav:click': function() {
	    					self.switchInOut($(this));
	    				},
	    				// 文本域输入内容，提交按钮的状态交互
	    				'.jsArea:input': function() {
	    					var textarea = this, el = $(textarea);
	    					// 求得提交按钮元素
	    					var elSubmit = el.parents('form').find('input[type="submit"]');
	    					// 对应的label元素按钮
	    					var elLabel = $('label[for="'+ elSubmit.attr('id') +'"]');
	    					// 合法性验证
	    					if (elSubmit.length && elLabel.length) {
	    						if ($.trim(textarea.value) == '' || $.validate.isOverflow(textarea)) {
	    							elSubmit.attr(DISABLED, DISABLED);
	    							elLabel.addClass(DISABLED);
	    						} else {
	    							elSubmit.removeAttr(DISABLED, DISABLED);
	    							elLabel.removeClass(DISABLED);
	    						}
	    					}
	    				},
	    				// IE7,IE8浏览器使用键盘触发
	    				'.jsArea:keyup': function() {
	    					if ('oninput' in document.createElement('div') == false) {
	    						$(this).trigger('input');
	    					}
	    				},
	    				// 添加emoji表情
	    				'.jsAddFace:click': function() {
	    					var trigger = $(this), textarea = trigger.parents('form').find('.jsArea');
	    					if (textarea.length) {
	    						common.emojis(trigger, textarea);
	    					}	    					
	    				},
	    				// 留言关闭
	    				'.jsCloseMsg:click': function() {
	    					self.switchInOut($(this));
	    				}
	    			},
	    			onShow: function() {
	    				this.el.table.find('form').each(function() {
	    					var form = $(this);
	    					new Form(form, {
	    						success: function(json) {
							        $.jsonHandle(json, function() {
							        	// 数据回填
							        	form.parents('tr').before(template('replyMsgTemp', json)).hide();
							        	form[0].reset();
							        	$.validate.count(form.find('textarea')[0]);
							        	// 列表的高度同步
	    								msgTable.el.size.unloading(0);
							        });
							    }
	     ...
	     * @return {[type]} [description]
	     */
	    replyTrToggle: function(trigger) {
	    	var rel = trigger && trigger.attr('data-rel');
	    	if (rel) {
	    		$('#' + rel).toggle();
	    	}
	    	return this;
	    ...
	    /**
	     * 加入精选和移出精选
	     * @param  {[type]} checkbox 点击的复选框元素
	     * @return {[type]}          [description]
	     */
	    switchInOut: function(checkbox) {
	    	var url = checkbox && checkbox.attr('data-url');

	    	if (url) {
	    		// 控件禁用，防止多次点击
	    		checkbox.attr(DISABLED, DISABLED);

	    		var data = $.queryToObject(checkbox.attr('data-params') || '');
	    		...
	    		$.ajax({
	    			...
	    			data: data,
	    			success: function(json) {
						$.lightTip.success("操作成功");
	    				$.jsonHandle(json, null, function() {
	    					// 错误发生
	    ...
	    /**
	     * 删除回复内容
	     * @param  {[type]}   btn       文字按钮，触发弹框的按钮
	     * @param  {[type]}   btnDialog 弹框内的按钮
	     * @param  {Function} callback  删除成功后的回调
	     * @return {[type]}             [description]
	     */
	    deleteReply: function(btn, btnDialog, callback) {
	    	var self = this;
...
var DATA_EMOJIS = [{name:"e056", title:"可爱",unicode:"U+1F60A"},
{name:"e057", title:"开心",unicode:"U+1F603",byte:"\xF0\x9F\x98\x83"},
{name:"e414", title:"害羞",unicode:"U+263A",byte:"\xE2\x98\xBA"},
{name:"e405", title:"挤眼",unicode:"U+1F609",byte:"\xF0\x9F\x98\x89"},
{name:"e106", title:"色",unicode:"U+1F60D",byte:"\xF0\x9F\x98\x8D"},
{name:"e418", title:"飞吻",unicode:"U+1F618",byte:"\xF0\x9F\x98\x98"},
{name:"e417", title:"亲亲",unicode:"U+1F61A",byte:"\xF0\x9F\x98\x9A"},
{name:"e40d", title:"发呆",unicode:"U+1F633",byte:"\xF0\x9F\x98\xB3"},
{name:"e40a", title:"思考",unicode:"U+1F606",byte:"\xF0\x9F\x98\x86"},
{name:"e404", title:"呲牙",unicode:"U+1F601",byte:"\xF0\x9F\x98\x81"},
{name:"e105", title:"调皮",unicode:"U+1F61C",byte:"\xF0\x9F\x98\x9C"},
{name:"e409", title:"赖皮",unicode:"U+1F61D",byte:"\xF0\x9F\x98\x9D"},
{name:"e40e", title:"无感",unicode:"U+1F612",byte:"\xF0\x9F\x98\x92"},
{name:"e402", title:"奸笑",unicode:"U+1F60F",byte:"\xF0\x9F\x98\x8F"},
{name:"e108", title:"汗",unicode:"U+1F613",byte:"\xF0\x9F\x98\x93"},
{name:"e403", title:"惆怅",unicode:"U+1F614",byte:"\xF0\x9F\x98\x94"},
{name:"e058", title:"伤心",unicode:"U+1F61E",byte:"\xF0\x9F\x98\x9E"},
{name:"e407", title:"崩溃",unicode:"U+1F616",byte:"\xF0\x9F\x98\x96"},
{name:"e401", title:"流汗",unicode:"U+1F625",byte:"\xF0\x9F\x98\xA5"},
{name:"e40f", title:"冷汗",unicode:"U+1F630",byte:"\xF0\x9F\x98\xB0"},
{name:"e40b", title:"衰",unicode:"U+1F628",byte:"\xF0\x9F\x98\xA8"},
{name:"e406", title:"头晕",unicode:"U+1F623",byte:"\xF0\x9F\x98\xA3"},
{name:"e413", title:"流泪",unicode:"U+1F622",byte:"\xF0\x9F\x98\xA2"},
{name:"e411", title:"大哭",unicode:"U+1F62D",byte:"\xF0\x9F\x98\xAD"},
{name:"e412", title:"笑抽",unicode:"U+1F602",byte:"\xF0\x9F\x98\x82"},
{name:"e410", title:"混乱",unicode:"U+1F632",byte:"\xF0\x9F\x98\xB2"},
{name:"e107", title:"恐怖",unicode:"U+1F631",byte:"\xF0\x9F\x98\xB1"},
{name:"e059", title:"生气",unicode:"U+1F620",byte:"\xF0\x9F\x98\xA0"},
{name:"e416", title:"发怒",unicode:"U+1F621",byte:"\xF0\x9F\x98\xA1"},
{name:"e408", title:"瞌睡",unicode:"U+1F62A",byte:"\xF0\x9F\x98\xAA"},
{name:"e40c", title:"感冒",unicode:"U+1F637",byte:"\xF0\x9F\x98\xB7"},
{name:"e11a", title:"魔鬼",unicode:"U+1F47F",byte:"\xF0\x9F\x91\xBF"},
{name:"e10c", title:"外星人",unicode:"U+1F47D",byte:"\xF0\x9F\x91\xBD"},
{name:"e022", title:"爱心",unicode:"U+2764",byte:"\xE2\x9D\xA4"},
{name:"e023", title:"心碎",unicode:"U+1F494",byte:"\xF0\x9F\x92\x94"},
{name:"e111", title:"接吻",unicode:"U+1F48F",byte:"\xF0\x9F\x92\x8F"},
{name:"e425", title:"爱情",unicode:"U+1F491",byte:"\xF0\x9F\x92\x91"},
{name:"e329", title:"爱神箭",unicode:"U+1F498",byte:"\xF0\x9F\x92\x98"},
{name:"e04a", title:"太阳",unicode:"U+2600",byte:"\xE2\x98\x80"},
{name:"e04b", title:"下雨",unicode:"U+2614",byte:"\xE2\x98\x94"},
{name:"e049", title:"多云",unicode:"U+2601",byte:"\xE2\x98\x81"},
{name:"e048", title:"雪人",unicode:"U+26C4",byte:"\xE2\x9B\x84"},
{name:"e04c", title:"月亮",unicode:"U+1F319",byte:"\xF0\x9F\x8C\x99"},
{name:"e13d", title:"闪电",unicode:"U+26A1",byte:"\xE2\x9A\xA1"},
{name:"e00e", title:"强",unicode:"U+1F44D",byte:"\xF0\x9F\x91\x8D"},
{name:"e420", title:"OK",unicode:"U+1F44C",byte:"\xF0\x9F\x91\x8C"},
{name:"e011", title:"胜利",unicode:"U+270C",byte:"\xE2\x9C\x8C"},
{name:"e231", title:"向右",unicode:"U+1F449",byte:"\xF0\x9F\x91\x89"},
{name:"e230", title:"向左",unicode:"U+1F448",byte:"\xF0\x9F\x91\x88"},
{name:"e41d", title:"拜佛",unicode:"U+1F64F",byte:"\xF0\x9F\x99\x8F"},
{name:"e41f", title:"鼓掌",unicode:"U+1F44F",byte:"\xF0\x9F\x91\x8F"},
{name:"e14c", title:"肌肉",unicode:"U+1F4AA",byte:"\xF0\x9F\x92\xAA"},
{name:"e11c", title:"骷髅",unicode:"U+1F480",byte:"\xF0\x9F\x92\x80"},
{name:"e536", title:"脚印",unicode:"U+1F463",byte:"\xF0\x9F\x91\xA3"},
{name:"e419", title:"眼睛",unicode:"U+1F440",byte:"\xF0\x9F\x91\x80"},
{name:"e04f", title:"猫咪",unicode:"U+1F431",byte:"\xF0\x9F\x90\xB1"},
{name:"e052", title:"狗狗",unicode:"U+1F436",byte:"\xF0\x9F\x90\xB6"},
{name:"e10b", title:"猪头",unicode:"U+1F437",byte:"\xF0\x9F\x90\xB7"},
{name:"e523", title:"黄鸟",unicode:"U+1F424",byte:"\xF0\x9F\x90\xA4"},
{name:"e054", title:"鲸",unicode:"U+1F433",byte:"\xF0\x9F\x90\xB3"},
{name:"e520", title:"海豚",unicode:"U+1F42C",byte:"\xF0\x9F\x90\xAC"},
{name:"e445", title:"南瓜灯",unicode:"U+1F383",byte:"\xF0\x9F\x8E\x83"},
{name:"e11b", title:"小鬼",unicode:"U+1F47B",byte:"\xF0\x9F\x91\xBB"},
{name:"e448", title:"圣诞老人",unicode:"U+1F385",byte:"\xF0\x9F\x8E\x85"},
{name:"e32e", title:"星星",unicode:"U+2728",byte:"\xE2\x9C\xA8"},
{name:"e13c", title:"睡觉",unicode:"U+1F4A4",byte:"\xF0\x9F\x92\xA4"},
{name:"e03e", title:"音乐",unicode:"U+1F3B5",byte:"\xF0\x9F\x8E\xB5"},
{name:"e110", title:"四叶草",unicode:"U+1F340",byte:"\xF0\x9F\x8D\x80"},
{name:"e033", title:"圣诞树",unicode:"U+1F384",byte:"\xF0\x9F\x8E\x84"},
{name:"e112", title:"礼物",unicode:"U+1F381",byte:"\xF0\x9F\x8E\x81"},
{name:"e325", title:"铃铛",unicode:"U+1F514",byte:"\xF0\x9F\x94\x94"},
{name:"e312", title:"礼花",unicode:"U+1F389",byte:"\xF0\x9F\x8E\x89"},
{name:"e310", title:"气球",unicode:"U+1F388",byte:"\xF0\x9F\x8E\x88"},
{name:"e10f", title:"灯泡",unicode:"U+1F4A1",byte:"\xF0\x9F\x92\xA1"},
{name:"e13f", title:"浴缸",unicode:"U+1F6C0",byte:"\xF0\x9F\x9B\x80"},
{name:"e140", title:"马桶",unicode:"U+1F6BD",byte:"\xF0\x9F\x9A\xBD"},
{name:"e12f", title:"金钱",unicode:"U+1F4B0",byte:"\xF0\x9F\x92\xB0"},
{name:"e311", title:"炸弹",unicode:"U+1F4A3",byte:"\xF0\x9F\x92\xA3"},
{name:"e113", title:"手枪",unicode:"U+1F52B",byte:"\xF0\x9F\x94\xAB"},
{name:"e03c", title:"唱歌",unicode:"U+1F3A4",byte:"\xF0\x9F\x8E\xA4"},
{name:"e13e", title:"高跟鞋",unicode:"U+1F460",byte:"\xF0\x9F\x91\xA0"},
{name:"e322", title:"内衣",unicode:"U+1F459",byte:"\xF0\x9F\x91\x99"},
{name:"e10e", title:"皇冠",unicode:"U+1F451",byte:"\xF0\x9F\x91\x91"},
{name:"e43c", title:"雨伞",unicode:"U+1F302",byte:"\xF0\x9F\x8C\x82"},
{name:"e31c", title:"口红",unicode:"U+1F484",byte:"\xF0\x9F\x92\x84"},
{name:"e034", title:"戒指",unicode:"U+1F48D",byte:"\xF0\x9F\x92\x8D"},
{name:"e035", title:"钻石",unicode:"U+1F48E",byte:"\xF0\x9F\x92\x8E"},
{name:"e42a", title:"篮球",unicode:"U+1F3C0",byte:"\xF0\x9F\x8F\x80"},
{name:"e018", title:"足球",unicode:"U+26BD",byte:"\xE2\x9A\xBD"},
{name:"e42c", title:"台球",unicode:"U+1F3B1",byte:"\xF0\x9F\x8E\xB1"},
{name:"e42d", title:"游泳",unicode:"U+1F3CA",byte:"\xF0\x9F\x8F\x8A"},
{name:"e01d", title:"飞机",unicode:"U+2708",byte:"\xE2\x9C\x88"},
{name:"e10d", title:"火箭",unicode:"U+1F680",byte:"\xF0\x9F\x9A\x80"},
{name:"e136", title:"单车",unicode:"U+1F6B2",byte:"\xF0\x9F\x9A\xB2"},
{name:"e42e", title:"汽车",unicode:"U+1F699",byte:"\xF0\x9F\x9A\x99"},
{name:"e159", title:"公交车",unicode:"U+1F68C",byte:"\xF0\x9F\x9A\x8C"},
{name:"e435", title:"动车",unicode:"U+1F684",byte:"\xF0\x9F\x9A\x84"},
{name:"e14e", title:"红绿灯",unicode:"U+1F6A5",byte:"\xF0\x9F\x9A\xA5"},
{name:"e252", title:"警告",unicode:"U+26A0",byte:"\xE2\x9A\xA0"},
{name:"e30f", title:"药丸",unicode:"U+1F48A",byte:"\xF0\x9F\x92\x8A"},
{name:"e045", title:"咖啡",unicode:"U+2615",byte:"\xE2\x98\x95"},
{name:"e047", title:"啤酒",unicode:"U+1F37A",byte:"\xE2\x98\x95"},
{name:"e30c", title:"对饮",unicode:"U+1F37B",byte:"\xF0\x9F\x8D\xBB"},
{name:"e120", title:"汉堡",unicode:"U+1F354",byte:"\xF0\x9F\x8D\x94"},
{name:"e33b", title:"薯条",unicode:"U+1F35F",byte:"\xF0\x9F\x8D\x9F"},
{name:"e33e", title:"米饭",unicode:"U+1F35A",byte:"\xF0\x9F\x8D\x9A"},
{name:"e340", title:"面条",unicode:"U+1F35C",byte:"\xF0\x9F\x8D\x9C"},
{name:"e339", title:"面包",unicode:"U+1F35E",byte:"\xF0\x9F\x8D\x9E"},
{name:"e34b", title:"生日蛋糕",unicode:"U+1F382",byte:"\xF0\x9F\x8E\x82"},
{name:"e348", title:"西瓜",unicode:"U+1F349",byte:"\xF0\x9F\x8D\x89"},
{name:"e158", title:"酒店",unicode:"U+1F3E8",byte:"\xF0\x9F\x8F\xA8"},
{name:"e43d", title:"爱心教堂",unicode:"U+1F492",byte:"\xF0\x9F\x92\x92"},
{name:"e449", title:"夕阳",unicode:"U+1F305"}];
...
$.fn.insertText = function(value) {
    ...
    if (document.selection) {
        ...
        if (control.selectionStart || control.selectionStart == '0') {
            var startPos = control.selectionStart, endPos = control.selectionEnd;
            var scrollTop = control.scrollTop;
            control.value = control.value.substring(0, startPos) + value + control.value.substring(endPos, control.value.length);
            this.focus();
            control.selectionStart = startPos + value.length;
            control.selectionEnd = startPos + value.length;
